...
const bg = computed(() => {
  ...
  const [a, b, c] = title.value.gradient
  return `linear-gradient(125deg, ${a} -10%, ${b} 45%, ${c} 110%)`
})
...
<template>
  <main v-if="title" class="detail">
    <section class="banner" :style="{ background: bg }">
      <button class="back" @click="router.back()">‹ Back</button>
      <div class="banner-shade" />
      <div class="banner-inner container-x">
        <div class="poster-side">
          <div class="poster-art" :style="{ background: bg }">
    ...
    <section v-if="episodes.length" class="episodes container-x">
      ...
      <ul>
        <li v-for="ep in episodes" :key="ep.n">
          ...
          <div class="ep-thumb" :style="{ background: bg }">
            <span>▶</span>
          </div>
          <div class="ep-body">
            <div class="ep-head">
...
.banner-shade {
  ...
  background:
    linear-gradient(180deg, rgba(7,7,13,0.35) 0%, rgba(7,7,13,0.6) 60%, rgba(7,7,13,1) 100%),
    radial-gradient(900px 600px at 80% 30%, rgba(0,0,0,0) 0%, rgba(7,7,13,0.7) 80%);
}
.banner-inner {
  position: relative;
...
.back:hover { background: rgba(7,7,13,0.85); }
...
.tag { font-style: italic; color: rgba(255,255,255,0.85); margin: 0 0 14px; }
.meta { display: flex; gap: 12px; align-items: center; margin-bottom: 16px; font-size: 0.92rem; color: rgba(255,255,255,0.85); }
.match { color: #4ade80; font-weight: 800; }
.rating-pill {
  border: 1px solid rgba(255,255,255,0.5);
  padding: 1px 8px; border-radius: 4px; font-size: 0.78rem;
}
.synopsis { max-width: 700px; line-height: 1.6; font-size: 1.05rem; margin: 0 0 22px; }
.actions { display: flex; gap: 10px; flex-wrap: wrap; margin-bottom: 26px; }
.credits { display: flex; flex-direction: column; gap: 6px; font-size: 0.9rem; color: rgba(255,255,255,0.78); }
.credits span { color: var(--muted); margin-right: 6px; }

.episodes { padding: 30px 24px 20px; }
.episodes h2 { font-size: 1.4rem; font-weight: 800; margin-bottom: 18px; }
.episodes ul { list-style: none; padding: 0; margin: 0; display: flex; flex-direction: column; gap: 10px; }
.episodes li {
  display: grid;
...
.episodes li:hover { background: rgba(255,255,255,0.05); cursor: pointer; }
.ep-num { font-size: 1.6rem; color: var(--muted); font-weight: 800; text-align: center; }
.ep-thumb {
  aspect-ratio: 16 / 9;
  border-radius: 8px;
  display: grid; place-items: center;
...
.ep-head { display: flex; justify-content: space-between; }
.ep-head h3 { margin: 0; font-size: 1.05rem; font-weight: 700; }
.ep-body p { margin: 6px 0 0; color: var(--muted); font-size: 0.9rem; }
...
@media (max-width: 820px) {
  ...
  .poster-side { position: static; max-width: 220px; }
  .episodes li { grid-template-columns: 28px 140px 1fr; gap: 12px; }
}
</style>
